...
export function SignInForm({ callbackUrl }: SignInFormProps) {
  ...
  const [isDeactivatedDialogDismissed, setIsDeactivatedDialogDismissed] =
    useState(false);
  const isDeactivatedDialogOpen =
    Boolean(state.deactivatedAccountDialog) && !isDeactivatedDialogDismissed;

  function handleFormAction(formData: FormData) {
    setIsDeactivatedDialogDismissed(false);
    formAction(formData);
  }
  ...
  return (
    <>
      <Card className="w-full max-w-md border-border bg-surface/95 text-text-strong shadow-card backdrop-blur">
        ...
        <CardContent>
          <form action={handleFormAction} className="space-y-md">
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open) {
            setIsDeactivatedDialogDismissed(true);
          }
        }}
      >
        {state.deactivatedAccountDialog ? (
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{state.deactivatedAccountDialog.title}</DialogTitle>
              <DialogDescription>
                {state.deactivatedAccountDialog.description}
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              <Button
                type="button"
                onClick={() => setIsDeactivatedDialogDismissed(true)}
              >
                Close
              </Button>
            </DialogFooter>
